import { image } from "./images";

export const postList = [
    {
        id: 1,
        title: '[이화-씨티] 글로벌금융실무론 이화여대 및 타교 청강생 모집(~3/8/2022)',
        writer: '경영대학행정실',
        date: '2022.02.18',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332872&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 1, name: '이화여자대학교', image: image.school.ewha } },
        starred: false,

    },
    {
        id: 2,
        title: '간호학전공 비사범대 교직과정 이수 신청 안내',
        writer: '교무처 학적팀',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332847&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 1, name: '이화여자대학교', image: image.school.ewha } },
        starred: false,

    },
    {
        id: 3,
        title: '[글로벌소통교육실] 2022-1 College English (for DIS students who are not freshmen only) 추가분반 개설 수요조사 안내',
        name: '컴퓨터공학과',
        writer: '글로벌소통교육실',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332813&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 1, name: '이화여자대학교', image: image.school.ewha } },
        starred: false,

    },
    {
        id: 4,
        title: '<고전읽기와글쓰기>, <나눔리더십>의 대체교과목 기수강자 중 재수강 신청 안내',
        content: '내용 내용내용 내용 내용냉냉냉냉냉냉 내용내용 내용입니다. 내용 내용내용 내용 내용냉냉냉냉냉냉 내용내용 내용입니다. 내용 내용내용 내용 내용냉냉냉냉냉냉 내용내용 내용입니다. 내용 내용내용 내용 내용냉냉냉냉냉냉 내용내용 내용입니다.',
        name: '컴퓨터공학과',
        writer: '호크마교양대학',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332703&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 1, name: '이화여자대학교', image: image.school.ewha } },
        starred: false,

    },
    {
        id: 5,
        title: ' [국제교류팀] 본교-홍콩 CUHK Global Student Partnership 학생단 모집',
        writer: '국제교류팀',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332780&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 3, name: '홍익대학교', image: image.school.hongik } },
        starred: false,

    },
    {
        id: 6,
        title: '[글로벌소통교육실] 2022-1 Advanced English(10099) (비영어권 외국인 반) 추가분반 개설 수요조사 안내 (구글폼)',
        name: '글로벌소통교육실',
        writer: '산학협력관',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332720&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 1, name: '이화여자대학교', image: image.school.ewha } },
        starred: false,

    },
    {
        id: 7,
        title: '제목제목제목 제목제목제목 제목제목제목 제목제목제목 제목제목제목 제목제목제목',
        writer: '산학협력관',
        date: '2022.01.04',
        url: 'http://cms.ewha.ac.kr/user/boardList.action?boardId=1631514&command=view2&boardSeq=52984466',
        board: { name: '컴퓨터공학과', school: { id: 2, name: '서강대학교', image: image.school.seogang } },
        starred: false,

    },
    {
        id: 8,
        title: '제목제목제목 제목제목제목 제목제목제목 제목제목제목 제목제목제목 제목제목제목',
        name: '컴퓨터공학과',
        writer: '산학협력관',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332720&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 1, name: '이화여자대학교', image: image.school.ewha } },
        starred: false,

    },
    {
        id: 9,
        title: '제목제목제목 제목제목제목 제목제목제목 제목제목제목 제목제목제목 제목제목제목',
        name: '컴퓨터공학과',
        writer: '산학협력관',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332720&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 1, name: '이화여자대학교', image: image.school.ewha } },
        starred: false,

    },
    {
        id: 10,
        title: '제목제목제목 제목제목제목 제목제목제목',
        name: '컴퓨터공학과',
        writer: '산학협력관',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332720&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 1, name: '이화여자대학교', image: image.school.ewha } },
        starred: false,

    },
    {
        id: 11,
        title: '제목제목제목 제목제목제목 제목제목제목',
        name: '컴퓨터공학과',
        writer: '산학협력관',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332720&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 1, name: '이화여자대학교', image: image.school.ewha } },
        starred: false,

    },
    {
        id: 12,
        title: '제목제목제목 제목제목제목 제목제목제목 제목제목제목 제목제목제목 제목제목제목',
        writer: '산학협력관',
        date: '2022.01.04',
        url: 'http://ewha.ac.kr/ewha/news/notice.do?mode=view&articleNo=332720&article.offset=0&articleLimit=10&srCategoryId1=25',
        board: { name: '컴퓨터공학과', school: { id: 3, name: '홍익대학교', image: image.school.hongik } },
        starred: false,

    },
]